...
printf("start\n");
	...
	if ((fp18 = fopen("ECG_data.out","w")) == NULL){ // pseudo-ECG data
		printf("Can't open File\n");
		exit(1);
	} 
	if ((fp19 = fopen("init_last.dat","w")) == NULL){ // initial values data at last time
		printf("Can't open File\n");
		exit(1);
	} 
	...
	// Location of unipolar electrogram
		...
		for(b_num=0; b_num < beats; b_num++){
			...
			for (j = 0; j< (div_num * BCL ); j++){
				...
				//if(time-(BCL*(double)b_num+100) >= 0.0 && time-(BCL*(double)b_num+100) < h ){
				if(time-(BCL*(double)b_num+200) >= 0.0 && time-(BCL*(double)b_num+200) < h ){
				//if(time-(BCL*(double)b_num+300) >= 0.0 && time-(BCL*(double)b_num+300) < h ){
				//if(time-(BCL*(double)b_num+400) >= 0.0 && time-(BCL*(double)b_num+400) < h ){
				//if(time-(BCL*(double)b_num+500) >= 0.0 && time-(BCL*(double)b_num+500) < h ){
				...
				//if(time-(BCL*(double)b_num+100.0) >= 0.0 && time-(BCL*(double)b_num+100.0) < 1.0){
				if(time-(BCL*(double)b_num+200.0) >= 0.0 && time-(BCL*(double)b_num+200.0) < 1.0){
				//if(time-(BCL*(double)b_num+300.0) >= 0.0 && time-(BCL*(double)b_num+300.0) < 1.0){
				//if(time-(BCL*(double)b_num+400.0) >= 0.0 && time-(BCL*(double)b_num+400.0) < 1.0){
				//if(time-(BCL*(double)b_num+500.0) >= 0.0 && time-(BCL*(double)b_num+500.0) < 1.0){
				...
				if(time-(BCL*(double)b_num) >= to_time){
					for(k=0;k<MEDIA_SITE;k++) gto_rate[k] = to_rate;
				}else{
					for(k=0;k<MEDIA_SITE;k++) gto_rate[k] = 1.0; 
				}
	if(debug){
